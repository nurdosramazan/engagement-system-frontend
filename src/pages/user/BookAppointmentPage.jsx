...
const BookAppointmentPage = () => {
    ...
    const startingDayIndex = useMemo(() => (start.getDay() + 6) % 7, [start]);
    ...
    const validateForm = () => {
        ...
        if (!formData.document) {
            newErrors.document = t('booking.validation.document_required');
        } else if (!ALLOWED_FILE_TYPES.includes(formData.document.type)) {
            newErrors.document = t('booking.validation.document_invalid_type');
        }

        if (formData.notes && formData.notes.length > MAX_NOTES_LENGTH) {
            newErrors.notes = t('booking.validation.notes_length', { count: MAX_NOTES_LENGTH });
        }
    ...
    const handleFileChange = (e) => {
        ...
        if (file) {
            if (!ALLOWED_FILE_TYPES.includes(file.type)) {
                setFormErrors(prev => ({ ...prev, document: t('booking.validation.document_invalid_type') }));
                e.target.value = null;
    ...
    const handleSubmit = (e) => {
        ...
        if (!selectedSlot) {
            toast.error(t('booking.validation.select_slot'));
            return;
        }
        if (!validateForm()) {
    ...
    const handleConfirmSubmit = (e) => {
        e.preventDefault();
        if (!hasAgreed) {
            toast.error(t('booking.validation.agree_rules'));
            return;
        }
    ...
    return (
        <div>
            <h1 className="text-3xl font-bold text-gray-800 mb-6">{t('booking.title')}</h1>
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                <div className="bg-white p-6 rounded-lg shadow-md">
                    <h2 className="text-xl font-semibold mb-4">{t('booking.calendar.title')}</h2>
                    <div className="mb-6">
                        <div className="flex items-center justify-between mb-4">
                            <button onClick={() => changeMonth(subMonths(currentMonth, 1))} className="px-2 py-1 rounded-md hover:bg-gray-100">&lt;</button>
                            <h3 className="font-semibold text-lg">{format(currentMonth, 'MMMM yyyy', { locale: currentLocale })}</h3>
                    ...
                    {selectedDate && (
                        <div>
                            <h3 className="font-semibold mb-3">{t('booking.calendar.available_slots', { date: format(selectedDate, 'MMMM d, yyyy', { locale: currentLocale }) })}</h3>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4 noValidate">
                        <div>
                            ...
                            {selectedSlot ? (
                                ...
                                <div className="flex items-center bg-gray-100 p-3 rounded-lg text-gray-500">
                                    <p className="text-sm">{t('booking.form.no_selection')}</p>
                                </div>
                            )}
                        </div>
                        ...
                        <div>
                            ...
                            <div className="flex items-center bg-blue-50 text-blue-700 text-sm p-3 rounded-lg mb-3">
                                <InfoIcon />
                                <span>{t('booking.form.witnesses_info')}</span>
                            </div>
                            {formData.witnesses.map((witness, index) => (
                                <div key={index} className="grid grid-cols-1 md:grid-cols-3 gap-2 mb-2 p-3 border rounded-md relative">
                                    <div>
                                        <input type="text" name="firstName" placeholder={t('booking.form.witness_first_name_placeholder')} value={witness.firstName} onChange={e => handleWitnessChange(index, e)} required
                                    ...
                                    <div>
                                        <input type="text" name="lastName" placeholder={t('booking.form.witness_last_name_placeholder')} value={witness.lastName} onChange={e => handleWitnessChange(index, e)} required
                            ...
                            {formData.witnesses.length < 3 && (
                                <button type="button" onClick={addWitness} className="text-sm text-indigo-600 hover:text-indigo-800 font-medium">{t('booking.form.add_witness')}</button>
                            )}
                        </div>

                        <div>
                            <label className="block text-sm font-medium text-gray-700">{t('booking.form.notes_label')}</label>
                            ...
                            <p className={`text-xs text-right mt-1 ${formData.notes.length >= MAX_NOTES_LENGTH ? 'text-red-500' : 'text-gray-500'}`}>
                                {t('booking.form.notes_chars_remaining', { count: MAX_NOTES_LENGTH - formData.notes.length })}
                            </p>
                            {formErrors.notes && <p className="mt-1 text-xs text-red-500">{formErrors.notes}</p>}
                        </div>

                        <div>
                            <label className="block text-sm font-medium text-gray-700">{t('booking.form.document_label')}</label>
                            <div className="flex items-center bg-yellow-50 text-yellow-800 text-sm p-3 rounded-lg my-2 border border-yellow-200">
                                <AlertTriangleIcon />
                                <span className="ml-2" dangerouslySetInnerHTML={{ __html: t('booking.form.document_warning') }}></span>
                            </div>
                            ...
                            {formData.document && <p className="mt-1 text-xs text-green-600">{t('booking.form.document_selected', { filename: formData.document.name })}</p>}
                        ...
                        {appointmentStatus === 'loading' && uploadProgress > 0 && (
                            <div className="w-full bg-gray-200 rounded-full mt-4">
                                ...
                                >
                                    {t('booking.form.upload_progress', { progress: uploadProgress })}
                                </div>
                            </div>
                        )}
                        <button type="submit" disabled={appointmentStatus === 'loading' || !selectedSlot} className="w-full bg-indigo-600 text-white py-3 px-4 rounded-md hover:bg-indigo-700 disabled:bg-indigo-300 font-semibold text-lg">
                            {appointmentStatus === 'loading' ? t('booking.form.button_loading') : t('booking.form.button_submit')}
                        </button>
                    </form>
                    <Modal
                    ...
                    >
                        <form onSubmit={handleConfirmSubmit}>
                            <div className="space-y-4">
                                ...
                                <div>
                                    <h4 className="font-semibold mb-2">{t('booking.modal.rules_title')}</h4>
                                    <div className="h-32 overflow-y-auto border p-3 text-sm text-gray-600">
                                        {t('booking.modal.rules', { returnObjects: true }).map((rule, index) => <p key={index}>{rule}</p>)}
                                    </div>
                                </div>
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <label htmlFor="agreeRules" className="text-sm font-medium text-gray-700">
                                        {t('booking.modal.agree_checkbox')}
                                    </label>
                                </div>
                            </div>

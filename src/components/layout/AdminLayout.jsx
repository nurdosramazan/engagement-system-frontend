        ...
        <div className="relative min-h-screen md:flex">
            ...
            <aside className={`fixed inset-y-0 left-0 bg-white shadow-lg w-64 transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:relative md:translate-x-0 transition-transform duration-300 ease-in-out z-30`}>
                ...
                <nav className="mt-6 px-4 space-y-2">
                    <NavLink to="/admin/dashboard" className={navLinkClasses}> <AdminDashboardIcon /> <span className="ml-4">Dashboard</span></NavLink>
                    <NavLink to="/admin/reports" className={navLinkClasses}> <ReportsIcon /> <span className="ml-4">Reports</span></NavLink>
                    <NavLink to="/admin/generate-slots" className={navLinkClasses}> <SlotsIcon /> <span className="ml-4">Generate slots</span></NavLink>
                </nav>
            </aside>
            ...
            <div className="flex-1 flex flex-col">
                <header className="bg-white shadow-md p-4 flex justify-between items-center sticky top-0 z-10">
                    ...
                    <div className="font-semibold text-gray-700 hidden md:block">Administrator View</div>
                    <div className="flex items-center space-x-4">
                        <Link to="/dashboard" className="flex items-center px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 font-semibold">
                            <UserViewIcon /> <span className="hidden sm:inline">Switch to user view</span>
                        </Link>
                        <NotificationBell />
